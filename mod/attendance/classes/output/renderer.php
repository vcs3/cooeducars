    ...
    protected function render_filter_controls(filter_controls $fcontrols) {
        ...
        if (empty($fcontrols->pageparams->studentid) && // Don't show add session button on user specific reports.
            ...
            $context->addsession = $this->output->single_button($url, get_string('addsession', 'attendance'), 'post',
             ...
             $urlparams = array('instanceid' => $fcontrols->urlparams['id']);
             $url = new moodle_url('/blocks/im_report/relatorio.php?curso='.$fcontrols->urlparams['id'],$urlparams);

             $context->addsession  .= $this->output->single_button($url,'Imprimir Diário', 'post',
             ['class' => 'addreport', 'primary' => true]);


             $urlparams = array('instanceid' => $fcontrols->urlparams['id']);
             $url_excel = new moodle_url('/blocks/im_report/relatorioFrequenciaXls.php',$urlparams);
             $context->addsession  .= $this->output->single_button($url,'Imprimir Excel', 'post',
             ['class' => 'addreport', 'primary' => true]);
